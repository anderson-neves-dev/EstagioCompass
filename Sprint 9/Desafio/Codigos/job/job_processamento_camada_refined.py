...
# Selecionando apenas a coluna paisOrigem para pegar os paises dos filmes
df_pais = df_tmdb.select("paisOrigem")

# Quebrando as linhas com multivalores separadas por vírgula
df_pais = df_pais.withColumn("paisOrigem", explode(split(col("paisOrigem"), ",")))

# Removendo linhas duplicadas 
df_pais = df_pais.dropDuplicates()

# Removendo linhas que estão sem valores
df_pais = df_pais.filter(col("paisOrigem") != "")

# Adicionando um numero de identidicação para cada pais
df_pais = df_pais.withColumn("id_pais", monotonically_increasing_id())

# Transformando a coluna 'paisOrigem' em múltiplas linhas, uma para cada pais
df_filme = df_tmdb.withColumn("paisOrigem", explode(split(col("paisOrigem"), ",")))

# Junção do dataframe de filmes com o df de pais 
