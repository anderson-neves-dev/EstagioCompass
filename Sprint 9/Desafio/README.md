...
- A primeira etapa foi analisar todos os meus dados contidos na camada Trusted Zone e gerar o modelo dimensional dos dados com a as tabelas dimensões e a tabela fato. 
- Meus dados inicialmente na camada trusted estavam da seguinte forma: 
![](Evidencias/esquema_dos_dados_na_camada_trusted.jpeg)
- Evidencia dos dados na camada trusted: 
![](Evidencias/print_envidencia_camada_trusted_dados_csv.png)
![](Evidencias/print_envidencia_camada_trusted_dados_tmdb.png)
- Evidencia de consultas dos dados da camada trusted no athena:
...
- Após analise dos dados da camada trusted e relacionar como eles iriam respoder as minhas questões, criei o seguinte modelo dimensional:
...
- Então, vou estar criando as seguintes tabelas com os atributos:
  ...
  - Fato filme:
    ...
    - Nota média do tmdb;
    - Nota média do imdb;
    - Numero de votos tmdb;
    - Numero de votos imdb;   
...
- Primeiramente, o código contém a importação das bibliotecas que irei utiliza
    ```python
    import sys
...
- Transformei a coluna 'paisOrigem' em múltiplas linhas, uma para cada pais no data frame com os dados tmdb, adicionando esses dados em um novo dataframe geral chamando df_filme que vou utilizar para juntar todos os dados ao decorrer da transformação dos dados.
    ...
    ```
- Realizo a junção do dataframe de df_filme com o df_pais através da coluna paisOrigem. Realizei essa junção com o método `inner` pois dessa forma filtro apenas os dados que tenho os paises de origem dos filmes que são dados relevantes para minha análise (apenas 5 filmes foram descartados).
    ```python
...
- Renomenado a coluna paisOrigem
    ...
    ```
- A próxima etapa do script, foi a de criar o dataframe responsável por armazenar as informações sobre gênero que foram o nome e o id. Pois, a coluna de generos que foi resgatado da api TMDB na etapa de ingestão de dados contém apenas os [IDs dos gêneros](Evidencias/print_dados_pais_e_genero_agrupados_como_lista.png). Esses dados foram extraídos da API TMDB no método gênero diretamente da documentação da API, pois esses valores são fixos e são apenas 19 itens e para padronizar decidir traduzi-los para português.
...
- Após isso, selecionei dos dados do CSV apenas as colunas: id, tituloPrincipal, tituloOriginal, notaMedia e numeroVotos. As duas últimas colunas contém dados provindos da plataforma de analise de filmes IMDB. Depois elimino as linhas duplicadas e realizo a junção com o dataframe geral dos dados.
    ```python
...
- Padronizando as colunas de id_genero e id_pais para o tipo integer nos dataframes que ss contém.
    ```python
...
- Por fim, salvo os dataframe dimensionais como arquivo parquet na camada Refined particinando de acordo a cada dataframe.
```python
...
- Com os dados armazenados na camada refined o proximo passo foi criar o crawler responsável por catalogar os meus dados da camada Refined e criar as tabelas em um database a partir dos meus dados padronizados em parquet.
- Evidencia de database criado nos serviços da AWS glue:
...
- Crawler criado com as seguintes configurações:
    ...
    - O data base de destino é de desafio-final-filmes-modelo-dimensional evidenciado acima, agendado sob demanda.
...
- Consultando todos os dados da tabela dimensão de filmes. Como resultado, obteve um total de 834 filmes, com a mesma quantidade do dataframe salvo.
![](Evidencias/print_evidencia_athena_consulta_dimensao_filmes.png)
- Consultando todos os dados da tabela dimensão de país. Como resultado, obteve um total de 60 países, com a mesma quantidade do dataframe salvo.
![](Evidencias/print_evidencia_athena_consulta_dimensao_pais.png)
- Consultando todos os dados da tabela dimensão de gênero. Como resultado, obteve um total de 19 gêneros, com a mesma quantidade do dataframe salvo.
![](Evidencias/print_evidencia_athena_consulta_dimensao_genero.png)
- Consultando todos os dados da tabela fato filmes. Como resultado, obteve um total de 3114 linhas, com a mesma quantidade do dataframe salvo.
