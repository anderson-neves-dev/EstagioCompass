...
source_file = args['S3_INPUT_PATH'] + '/2024/09/06/*.json'
...
df_movies = df_movies.select("imdb_id", "title","release_date","budget","revenue", "popularity", "vote_average", "vote_count", "poster_path" , "genre_ids")

# Convertendo tipos arrays para mesma linha separando por vírgula 
df_movies = df_movies.withColumn("genre_ids", concat_ws(",", col("genre_ids")))
...
df_movies = df_movies.withColumn("receita", col("receita").cast(DecimalType(38, 2)))\
                     .withColumn("orcamento", col("orcamento").cast(DecimalType(38, 2)))\
...
df_movies = df_movies.dropna(subset=['id','orcamento', 'receita', 'numeroVotosTMDB', 'notaMediaTMDB', 'popularidadeTMDB'])
